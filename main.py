...
def get_video_id(message):
    if message.video:
        bot.send_message(message.chat.id, f"VIDEO file_id:\n{message.video.file_id}")
    elif message.document:
        bot.send_message(message.chat.id, f"DOCUMENT file_id:\n{message.document.file_id}")
...
def start(message):
    ...
    keyboard.row(InlineKeyboardButton(
        "Играть 🎮",
        web_app=telebot.types.WebAppInfo("https://snusfnf-png.github.io/cardrop/")
    ))
    keyboard.row(
